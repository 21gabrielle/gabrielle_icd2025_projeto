...
```{r}
dados_expectativa <- select(gapminder, -pop, -gdpPercap)
dados_expectativa
...
head(dados %>% select(country, gdpPercap, pop, PIB_total))
```
...
# Exibir algumas linhas do resultado
head(dados %>% select(country, pop, economia_grande))
```
...
paises_classificados <- gapminder %>%
  ...
  mutate(
    classe_desenvolvimento = case_when(
      gdpPercap < 60 ~ "Baixa" ,
      gdpPercap >= 60 & gdpPercap <= 75 ~ "Médio" ,
      gdpPercap > 10000 ~ "Alto"
    )
  ) %>%
  select(country, gdpPercap, classe_desenvolvimento) %>%
...
gapminder %>%
  filter(year == 2007) %>%
  summarise(
    abaixo_60 = sum(gdpPercap < 60),
    entre_60_75 = sum(gdpPercap >= 60 & gdpPercap <= 75),
    acima_10000 = sum(gdpPercap > 10000)
  )
```
...
```{r}
tabela_filiais <- gapminder %>%
  filter(year == 2007) %>%
  group_by(continent) %>%
  summarize(
    numero_filiais = n(),  # Número de países (filiais)
    pib_per_capita_medio = mean(gdpPercap),  # PIB per capita médio
    melhor_filial_pib = max(gdpPercap)  # PIB per capita máximo
  )
...
```{r}
expectativa_americana <- gapminder %>%
  filter(continent == "Americas") %>%
  group_by(year) %>%
  summarize(expectativa_media = mean(lifeExp)) %>%
  arrange(year)  # Ordena os anos de forma crescente

# Exibir a tabela
expectativa_americana
```
...
# Exibir os primeiros países ordenados
head(paises_ordenados %>% select(country, lifeExp))
```
...
```{r}
# Filtrar o ano de 2007 e ordenar pelo menor PIB per capita
...
```{r}
# Filtrar países das Américas e ordenar por população
...
# Exibir o ranking dos continentes
ranking_continentes
```
