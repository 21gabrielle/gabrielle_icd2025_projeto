...
retorno_esperado_A <- sum(retornos_A * prob_A)
lucro_esperado_A <- retorno_esperado_A - investimento_A
print(paste("Lucro esperado Projeto A: R$", lucro_esperado_A))

# Projeto B
retornos_B <- c(180000, 260000, 400000)
prob_B <- c(0.2, 0.5, 0.3)
investimento_B <- 200000

retorno_esperado_B <- sum(retornos_B * prob_B)
lucro_esperado_B <- retorno_esperado_B - investimento_B
print(paste("Lucro esperado Projeto B: R$", lucro_esperado_B))
```


2. Qual projeto escolheria baseado apenas no valor esperado?

Baseando-se apenas no valor esperado, você escolheria o Projeto B (Arriscado). Ele apresenta um lucro esperado de R$ 86.000, que é superior aos R$ 74.000 de lucro esperado do Projeto A.

3. Que informações adicionais seriam úteis para decidir?

Embora o lucro esperado seja uma métrica fundamental, depender apenas dele pode ser enganoso, especialmente quando há incerteza envolvida. Para tomar uma decisão verdadeiramente informada sobre os projetos A e B, sua empresa se beneficiaria ao considerar vários outros fatores. Primeiramente, é crucial entender a tolerância a risco da empresa. O Projeto B, embora prometa um lucro esperado maior, também carrega a possibilidade de uma perda real em seu pior cenário, enquanto o Projeto A, mais conservador, garante algum lucro mesmo na situação menos favorável.

A análise do pior cenário e do melhor cenário para cada projeto oferece uma visão mais clara dos extremos. No caso do Projeto A, o pior resultado ainda gera um lucro de R$ 20.000, enquanto o Projeto B pode resultar em uma perda de R$ 20.000. Por outro lado, o potencial de ganho do Projeto B é significativamente maior, podendo chegar a um lucro de R$ 200.000, em contraste com o lucro máximo de R$ 120.000 do Projeto A. Essa amplitude de resultados é uma indicação direta da volatilidade e do risco de cada investimento.

Para quantificar esse risco, seria extremamente útil calcular o desvio padrão ou a variância dos retornos de cada projeto. Essas medidas estatísticas fornecem uma noção da dispersão dos resultados possíveis. Um projeto com maior desvio padrão é intrinsecamente mais arriscado, pois seus retornos esperados podem variar amplamente.

Além disso, é importante considerar a correlação com outros investimentos que a empresa já possui. Um projeto que, à primeira vista, parece menos lucrativo individualmente, pode ser extremamente valioso se ajudar a diversificar o portfólio geral da empresa, reduzindo o risco total. As necessidades de liquidez da empresa também são um ponto a ser avaliado; entender se os fundos investidos podem ser facilmente acessados ou se estarão "presos" por um longo período é vital para o planejamento financeiro.

Finalmente, mas não menos importante, a decisão deve estar alinhada com o ajuste estratégico do projeto. Um investimento que se encaixa perfeitamente nos objetivos de longo prazo e na missão da empresa pode ser preferível, mesmo que seu lucro esperado seja marginalmente menor em comparação com outra opção. Às vezes, os benefícios estratégicos superam ligeiras diferenças no retorno financeiro puro. Ao integrar todas essas perspectivas – risco, cenários extremos, diversificação, liquidez e alinhamento estratégico – sua empresa poderá fazer uma escolha de investimento muito mais robusta e alinhada com seus objetivos gerais.
