...
```{r}
# Calcula a média de vendas por categoria e ordena do maior para o menor
media_vendas_categoria <- dados_vendas_longo %>%
  group_by(categoria) %>%
  summarize(media_vendas = mean(valor_vendas)) %>%
...
```{r}
# Filtra os produtos da categoria "Mobiliário" e calcula as vendas totais por produto
...
```{r}
# Analisa o desempenho mensal por categoria
desempenho_mensal_categoria <- dados_vendas_longo %>%
  group_by(categoria) %>%
  summarize(
    vendas_total = sum(valor_vendas),
    vendas_media = mean(valor_vendas),
