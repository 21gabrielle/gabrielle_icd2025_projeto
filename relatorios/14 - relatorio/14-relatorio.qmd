...
A população é o conjunto total de todas as garrafas de vinho que a empresa possui. Neste caso, são as 8.000 garrafas de vinho.

2. Qual é a amostra?
A amostra é o subconjunto da população selecionado para avaliação. Neste caso, são as 120 garrafas que o analista seleciona.

3. O que representa $\mu$?
μ (mi) representa a média da população. No contexto do cenário, seria a média da qualidade de todas as 8.000 garrafas de vinho (um valor teórico ou ideal, pois não todas são avaliadas).

4. O que representa $\bar{x}$?
  (x barra) representa a média da amostra. No cenário, seria a média da qualidade das 120 garrafas de vinho selecionadas para avaliação pelos especialistas.
  
5. Por que não avaliar todas as garrafas?
Custo: Avaliar 8.000 garrafas por especialistas seria extremamente caro em termos de tempo e recursos financeiros.
Tempo: O processo de avaliação de qualidade para cada garrafa levaria muito tempo, o que pode não ser viável para decisões rápidas de produção ou venda.
Viabilidade/Destrutividade: A avaliação de qualidade de vinhos muitas vezes envolve degustação ou testes que consomem o produto, tornando inviável ou indesejável avaliar a população inteira, pois as garrafas seriam inutilizadas para venda.
Inferência: A amostragem permite tirar conclusões válidas sobre a população inteira a partir de um subconjunto menor, com um nível aceitável de precisão e confiança, sem a necessidade de inspecionar cada item.
...
1. Qual é a população? 1.200 farmacias
...
Extraindo uma amostra aleatoria simples com a função slice_sample do dplyr:
```{r}
# fixa a semente para reproduzir
set.seed(234)
#extral AAS dos municipios
