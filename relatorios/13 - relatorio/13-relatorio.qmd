...
```{r}
# 1. Preparação da tabela de frequência do estado civil
#2. Crie um gráfico de barras horizontais básico:
tabela_estado_civil_frequencia <- questionario %>%
  count(estado_civil) %>%
  arrange(desc(n)) %>% # Ordena em ordem DECRESCENTE
  mutate(estado_civil = factor(estado_civil, levels = estado_civil))

# Exibe a tabela (como o você solicitou anteriormente, agora com o nome corrigido)
tabela_estado_civil_frequencia

# 2. Criação e exibição do gráfico de barras melhorado
grafico_barras_estado_civil_melhorado <-
  ggplot(tabela_estado_civil_frequencia, aes(x = estado_civil, y = n)) +
  geom_bar(stat = "identity", fill = "steelblue") + # Adicionei uma cor para as barras
  coord_flip() + # Inverte os eixos para barras horizontais
  geom_text(aes(label = n), hjust = -0.3, size = 4) + # Adiciona os rótulos de frequência
  labs(x = "Estado Civil",
       y = "Número de Respondentes",
       title = "Distribuição dos Respondentes por Estado Civil") +
  theme_minimal() # Define um tema minimalista para o gráfico

# Exibe o gráfico (essa linha é crucial para que o gráfico apareça no R Markdown)
grafico_barras_estado_civil_melhorado
```
...
# Criação e exibição do gráfico de barras melhorado
grafico_barras_estado_civil_melhorado <-
  ggplot(tabela_estado_civil_frequencia, aes(x = estado_civil, y = n)) +
  geom_bar(stat = "identity", fill = "steelblue") + # Adicionei uma cor para as barras
  coord_flip() + # Inverte os eixos para barras horizontais
  
  # Adiciona os valores da frequência absoluta nas barras
  geom_text(aes(label = n), 
            hjust = -0.3, # Posiciona o texto um pouco à direita das barras horizontais
            size = 4) +   # Define o tamanho do texto
  
  # Adiciona Título e rótulos apropriados dos eixos
  labs(x = "Estado Civil", 
       y = "Número de Respondentes", 
       title = "Distribuição dos Respondentes por Estado Civil") +
  
  # Aplica o tema minimalista
  theme_minimal()
...
# Para criar uma tabela visual formatada, use knitr::kable()
knitr::kable(head(dados_acoes), 
             caption = "Primeiras Linhas dos Dados de Ações (PETR4.SA e ITUB4.SA)")
```
